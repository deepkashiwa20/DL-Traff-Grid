...
def test_model(X_test, Y_test):
    ...
    pred = model.predict(X_test, verbose=1, batch_size=BATCHSIZE) * MAX_FLOWIO
    groundtruth = Y_test * MAX_FLOWIO
    np.save(PATH + '/' + MODELNAME + '_prediction.npy', pred)
    np.save(PATH + '/' + MODELNAME + '_groundtruth.npy', groundtruth)

################# Path Setting #######################
MODELNAME = 'DeepSTN+'
KEYWORD = 'predflowio_DeepSTN+_1907272056'
# KEYWORD = 'predflowio_DeepSTN+_1907281454'
# KEYWORD = 'predflowio_DeepSTN+_1907281848'
PATH = '../' + KEYWORD
###########################Reproducible#############################
import numpy as np
import random
