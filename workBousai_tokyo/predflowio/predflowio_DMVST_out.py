...
def get_data(model_name, type):
    ...
    trainData = region_window[startIndex * DAYTIMESTEP:(endIndex + 1) * DAYTIMESTEP, :, :, :, :]
    trainTemporal = temporal_data[startIndex * DAYTIMESTEP:(endIndex + 1) * DAYTIMESTEP]
    print('train data', trainData.shape)

    startIndex, endIndex = dates.index(validStartDate), dates.index(validEndDate)
    validData = region_window[startIndex * DAYTIMESTEP:(endIndex + 1) * DAYTIMESTEP, :, :, :, :]
    validTemporal = temporal_data[startIndex * DAYTIMESTEP:(endIndex + 1) * DAYTIMESTEP]
    print('valid data', validData.shape)

    startIndex, endIndex = dates.index(testStartDate), dates.index(testEndDate)
    testData = region_window[startIndex * DAYTIMESTEP:(endIndex + 1) * DAYTIMESTEP, :, :, :, :]
    testTemporal = temporal_data[startIndex * DAYTIMESTEP:(endIndex + 1) * DAYTIMESTEP]
...
def model_train(model_name, train_data, valid_data, trainTemporal, validTemporal, topo_data, type):
    ...
    # mse
    scaled_valY = np.reshape(valY, ((valid_data.shape[0] - TIMESTEP), REGION_SIZE, REGION_SIZE))
    scaled_predValY = np.reshape(val_predY, ((valid_data.shape[0] - TIMESTEP), REGION_SIZE, REGION_SIZE))
    ...
    # rescale mse
    valY, val_predY = valY * MAX_VALUE, val_predY * MAX_VALUE
    valY = np.reshape(valY, ((valid_data.shape[0] - TIMESTEP), REGION_SIZE, REGION_SIZE))
    val_predY = np.reshape(val_predY, ((valid_data.shape[0] - TIMESTEP), REGION_SIZE, REGION_SIZE))
    print('val rescale shape: ', val_predY.shape)
    val_rescale_MSE = np.mean((valY - val_predY) ** 2)
...
def model_pred(model_name, test, testTemporal, topo_data, type):
    ...
    # compute mse
    scaled_testY = np.reshape(testY, ((test.shape[0] - TIMESTEP), REGION_SIZE, REGION_SIZE))
    scaled_predTestY = np.reshape(predY, ((test.shape[0] - TIMESTEP), REGION_SIZE, REGION_SIZE))
    ...
    retestY = np.reshape(retestY, ((test.shape[0] - TIMESTEP), REGION_SIZE, REGION_SIZE))
    repredTestY = np.reshape(repredTestY, ((test.shape[0] - TIMESTEP), REGION_SIZE, REGION_SIZE))
...
KEYWORD = 'predflowio_DMVST_1906031534'
PATH = '../' + KEYWORD
###########################Reproducible#############################
import numpy as np
import random
...
if __name__ == '__main__':
    ...
    val_sc, val_re, test_sc, test_re = [2.979103831956276e-05], [23.438665427608704], [2.929438802383039e-05], [23.047916367943756]

    for type in ['out']:
        StartTime = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        train_data, valid_data, test_data, trainTemporal, validTemporal, testTemporal, topo_data = get_data(model_name,
                                                                                                            type)
        val_scale, val_rescale = model_train(model_name, train_data, valid_data, trainTemporal, validTemporal, topo_data, type)
