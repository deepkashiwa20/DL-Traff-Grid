...
def get_data(model_name):
    ...
    startIndex, endIndex = int(int(region_window.shape[0] * trainRatio) * (1 - SPLIT)) - empty_time, int(
...
def model_pred(test_data):
    ...
    pred = np.reshape(pred, ((test_data.shape[0] - empty_time), HEIGHT, WIDTH, DATACHANNEL))
    np.save(PATH + '/' + MODELNAME + '_prediction.npy', pred * MAX_VALUE)

    testY = get_test_true(test_data)
    testY = np.reshape(testY, ((test_data.shape[0] - empty_time), HEIGHT, WIDTH, DATACHANNEL))
...
KEYWORD = 'predflowio_STDN_1908120927'
# KEYWORD = 'predflowio_STDN_1908162159'
# KEYWORD = 'predflowio_STDN_1908171639'
PATH = '../' + KEYWORD
###########################Reproducible#############################
import numpy as np
import random
